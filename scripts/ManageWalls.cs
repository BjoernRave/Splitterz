...
public class ManageWalls : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (WallList.Count == ActiveWalls.Count && WallList.Count != 0)
        {
            ...
            WallList.Clear();
            if (GameObject.Find("WallModul(Clone)") != null)
            {
        ...
        if (StopWallLT.IsCollidedLT && StopWallRD.IsCollidedRD && !StopWallLT.Splitting && !StopWallRD.Splitting || Shortestpath3.NumberPossible)
        {
            if (GameObject.Find("PreviewHoriz(Clone)") != null)
            {
                GameObject.Find("PreviewHoriz(Clone)").layer = 4;
            }
            if (GameObject.Find("Player(Clone)") != null)
            {
                GameObject.Find("Player(Clone)").layer = 4;
            }
            WallList.AddRange(GameObject.FindGameObjectsWithTag("TopWalls"));
            WallList.AddRange(GameObject.FindGameObjectsWithTag("LeftWalls"));
            WallList.AddRange(GameObject.FindGameObjectsWithTag("RightWalls"));
            ...
            Shortestpath3.NumberPossible = false;
        }
...

    {
        ActiveWalls.Clear();
        WallList.Clear();
        Shortestpath3.FillWholeField = false;
        ...
        Shortestpath3.NewWallX = 0;
        Shortestpath3.NewWallY = 0;
        StopWallLT.IsCollidedLT = false;
        StopWallRD.IsCollidedRD = false;
